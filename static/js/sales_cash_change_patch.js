(() => {
    ...
    const originalOpenCashChangeModal = window.openCashChangeModal;
    const originalCreateInvoiceSale = window.createInvoiceSale;
    const originalPayKaspiPOS = window.payKaspiPOS;
    ...
    async function loadInsufficientStockItems() {
    ...
    function showNegativeStockWarning(items) {
        return new Promise(resolve => {
            ...
            style.textContent = `
                .negative-stock-sale-modal{position:fixed;inset:0;z-index:2147483000;display:flex;align-items:center;justify-content:center;padding:20px;background:rgba(17,24,39,.58);backdrop-filter:blur(3px)}
                .negative-stock-sale-dialog{width:min(540px,100%);max-height:min(680px,calc(100dvh - 40px));display:flex;flex-direction:column;overflow:hidden;background:#fff;border-radius:20px;box-shadow:0 24px 70px rgba(17,24,39,.28)}
            ...
            modal.innerHTML = `
                <div class="negative-stock-sale-dialog" role="dialog" aria-modal="true" aria-labelledby="negativeStockSaleTitle">
                    <div class="negative-stock-sale-head">
                        ...
                        <p>Количества на складе не хватает для этой операции. Можно отменить или продолжить всё равно.</p>
                    </div>
                        ...
                        ${items.map(item => `
                            <div class="negative-stock-sale-item">
                                <div>
                                    ...
                                    <small>Количество: ${formatQty(item.requested)} ${escapeText(item.unit || '')}</small>
                                </div>
                                <div class="negative-stock-sale-balance">
                                    Сейчас: ${formatQty(item.stock)} ${escapeText(item.unit || '')}
                                    <b>После операции: ${formatQty(item.projected_stock)} ${escapeText(item.unit || '')}</b>
                                </div>
                            </div>
                        `).join('')}
                    </div>
                    <div class="negative-stock-sale-actions">
                        <button type="button" class="negative-stock-sale-cancel">Отмена</button>
                        <button type="button" class="negative-stock-sale-confirm">Продолжить всё равно</button>
                    </div>
                </div>
            `;
    ...
    async function confirmStockForCurrentCart() {
        ...
        guardPromise = (async () => {
            try {
                const items = await loadInsufficientStockItems();
                ...
                alert('Не удалось проверить остаток товара. Оплата не начата.');
                return false;
            } finally {
                guardPromise = null;
            }
    ...
    if (typeof originalOpenCashChangeModal === 'function') {
        window.openCashChangeModal = async (...args) => {
            if (!(await confirmStockForCurrentCart())) return;
            return originalOpenCashChangeModal(...args);
        };
    }
    ...
    if (typeof originalCreateInvoiceSale === 'function') {
        window.createInvoiceSale = async (...args) => {
            if (!(await confirmStockForCurrentCart())) return;
            approvedSignature = null;
            return originalCreateInvoiceSale(...args);
        };
    }

    if (typeof originalPayKaspiPOS === 'function') {
        window.payKaspiPOS = async (...args) => {
            if (!(await confirmStockForCurrentCart())) return;
            return originalPayKaspiPOS(...args);
        };
    }
