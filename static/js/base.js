...
function navigateApp(url) {
    if (typeof window.nikaShellNavigate === 'function') window.nikaShellNavigate(url);
...
async function openNotification(id, link) {
    ...
    if (link) navigateApp(link); else loadNotifications();
}
...
document.addEventListener('DOMContentLoaded',()=>{
    if(window.NIKA_EMBEDDED_PAGE)return;
    restoreSidebarState();
    setActiveMenuItem();
